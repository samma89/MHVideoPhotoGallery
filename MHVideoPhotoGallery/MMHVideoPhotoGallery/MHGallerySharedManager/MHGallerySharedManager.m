...
-(void)getYoutubeThumbImage:(NSString*)URL
    ...
    if (image) {
        ...
        NSString *videoID = [[URL componentsSeparatedByString:@"?v="] lastObject];
        NSString *youTubeImageBaseUrl = @"http://img.youtube.com/vi/";
        NSString *youTubeThumbUrl = [NSString stringWithFormat:@"%@%@/mqdefault.jpg",youTubeImageBaseUrl,videoID];
        
     /*
        dispatch_async(dispatch_get_main_queue(), ^(void){
            
                [SDWebImageManager.sharedManager downloadImageWithURL:[NSURL URLWithString:youTubeThumbUrl]
                                                              options:SDWebImageContinueInBackground
                                                             progress:nil
                                                            completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                                                                
                                                                [SDImageCache.sharedImageCache removeImageForKey:youTubeThumbUrl];
                                                                [SDImageCache.sharedImageCache storeImage:image
                                                                                                   forKey:URL];
                                                                
                                                                succeedBlock(image,(int)10,nil);
                                                            }];
...
//        NSString *infoURL = [NSString stringWithFormat:MHYoutubeInfoBaseURL,videoID];
        NSString *infoURL = [NSString stringWithFormat:@"%@%@&part=contentDetails&key=%@",MHYoutubeInfoBaseURL,videoID,YoutubeAPIKEY];
                                   ...
                                   if (!connectionError) {
                                       ...
                                       dispatch_async(dispatch_get_main_queue(), ^(void){
                                           if (jsonData.count) {
                                             ...
                                             id contentDetailArray =  [arr objectAtIndex:0];
                                             id contentDetail = [contentDetailArray objectForKey:@"contentDetails"];
                                             NSString *duration = [contentDetail objectForKey:@"duration"];
                                              NSUInteger timeSecond =  [self integerFromYoutubeDurationString:duration];
                                               
                                               
                                               NSMutableDictionary *dictToSave = [self durationDict];
                                               dictToSave[URL] = @(timeSecond);
                                               [self setObjectToUserDefaults:dictToSave];
                                               ...
                                               [SDWebImageManager.sharedManager downloadImageWithURL:[NSURL URLWithString:thumbURL]
                                                                                           ...
                                                                                           completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
                                                                                               ...
                                                                                               succeedBlock(image,timeSecond,nil);
                                                                                           }];
                                           }
                                       });
                                   }else{
